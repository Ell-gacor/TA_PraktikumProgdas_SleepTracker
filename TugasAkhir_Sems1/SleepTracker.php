...
class SleepTracker {
  ...
  public function getTip($hours) {
    ...
    public function checkAndResetWeekly() {
        ...
        // Jika hari ini adalah Senin (1) dan belum reset minggu ini
        if($today->format('N') == 1) { // 1 = Monday
            if($lastResetDate) {
                $lastReset = new DateTime($lastResetDate);
                $lastReset->setTime(0, 0, 0);
            } else {
                $lastReset = new DateTime('1970-01-01');
            }
            
            // Jika sudah lebih dari 1 minggu sejak reset terakhir
            $diff = $today->diff($lastReset);
            if($diff->days >= 7) {
                // Reset stack dan notes (simpan sebagai historical data)
                $this->stack = [];
                $this->notes = [];
                $this->queue = [];
