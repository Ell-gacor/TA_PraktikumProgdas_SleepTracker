...
class SleepTracker {
    ...
    private function loadFromJSON() {
        ...
        if($data && isset($data[$this->username])) {
            ...
            $this->archivedWeeks = $userData['archived_weeks'] ?? [];
        }
    }
    ...
    public function saveToJSON() {
        ...
        // Pertahankan field password, created_at, last_reset, archived_weeks, leaderboard_reset_date jika sudah ada
        ...
        $data[$this->username] = [
            ...
            'last_reset' => $existingUserData['last_reset'] ?? '',
            'leaderboard_reset_date' => $existingUserData['leaderboard_reset_date'] ?? '',
            'archived_weeks' => $existingUserData['archived_weeks'] ?? [],
            'stack' => $this->stack,
            'notes' => $this->notes,
            'queue' => $this->queue
        ];
  ...
  public function getTip($hours) {
    ...
    // leaderboard - hanya menampilkan data minggu ini (sejak reset terakhir)
    public function getLeaderboard() {
        // Baca data dari JSON untuk cek leaderboard_reset_date
        $leaderboardResetDate = null;
        if(file_exists($this->dataFile)) {
            $jsonData = json_decode(file_get_contents($this->dataFile), true);
            $leaderboardResetDate = $jsonData[$this->username]['leaderboard_reset_date'] ?? null;
        }
        
        // Filter stack untuk hanya menampilkan data setelah leaderboard reset
        $leaderboardData = [];
        if(!empty($leaderboardResetDate)) {
            $resetDate = new DateTime($leaderboardResetDate);
            foreach($this->stack as $item) {
                $itemDate = new DateTime($item['date']);
                if($itemDate >= $resetDate) {
                    $leaderboardData[] = $item;
                }
            }
        } else {
            $leaderboardData = $this->stack;
        }
        
        usort($leaderboardData, function($a, $b) { return $b["duration"] - $a["duration"]; });
    ...
    // Check dan reset weekly data setiap senin
    public function checkAndResetWeekly() {
        // Ambil last reset date dari JSON
        $data = [];
        if(file_exists($this->dataFile)) {
            $data = json_decode(file_get_contents($this->dataFile), true);
        }
        
        $lastResetDate = $data[$this->username]['last_reset'] ?? null;
        $today = new DateTime();
        $today->setTime(0, 0, 0);
        
        // Jika hari ini adalah Senin (1)
        if($today->format('N') == 1) { // 1 = Monday
            if($lastResetDate) {
                $lastReset = new DateTime($lastResetDate);
                $lastReset->setTime(0, 0, 0);
                // Hanya reset jika sudah lebih dari 1 minggu sejak reset terakhir
                $diff = $today->diff($lastReset);
                if($diff->days >= 7) {
                    // Archive current week's data sebelum reset
                    if(!empty($this->stack) || !empty($this->notes)) {
                        $weekStart = clone $lastReset;
                        $weekEnd = clone $lastReset;
                        ...
                        if(!isset($data[$this->username]['archived_weeks'])) {
                            $data[$this->username]['archived_weeks'] = [];
                        }
                        array_unshift($data[$this->username]['archived_weeks'], $archivedWeek);
                    }
                    
                    // Reset stack dan notes
                    $this->stack = [];
                    $this->notes = [];
                    $this->queue = [];
                    
                    // Update last_reset date dan save immediately
                    if(!isset($data[$this->username])) {
                        $data[$this->username] = [];
                    }
                    $data[$this->username]['last_reset'] = $today->format('Y-m-d');
                    $data[$this->username]['leaderboard_reset_date'] = $today->format('Y-m-d');
                    $data[$this->username]['stack'] = [];
                    $data[$this->username]['notes'] = [];
                    $data[$this->username]['queue'] = [];
                    
                    file_put_contents($this->dataFile, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
                }
            } else {
                // First time Monday, set last_reset dan leaderboard_reset_date
                if(!isset($data[$this->username])) {
                    $data[$this->username] = [];
                }
                $data[$this->username]['last_reset'] = $today->format('Y-m-d');
